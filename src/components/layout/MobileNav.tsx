...
export const MobileNav: React.FC<MobileNavProps> = ({ activeTab, onTabChange }) => {
  ...
  return (
    <nav aria-label="Mobilnavigasjon" className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-[#070b16]/95 backdrop-blur-2xl border-t border-slate-800/90 px-1 pt-1 pb-[calc(env(safe-area-inset-bottom,0px)+0.35rem)] shadow-2xl shadow-black">
      <div className="grid w-full grid-cols-7 items-stretch gap-0.5">
        {tabs.map((tab) => {
          const Icon = tab.icon;
          ...
          return (
            <button
              ...
              className={`flex min-h-12 min-w-0 flex-col items-center justify-center rounded-xl px-0.5 py-1.5 transition-all duration-150 active:scale-95 ${
                isActive
            ...
            >
              ...
              <span className="w-full truncate text-center text-[9px] leading-tight tracking-tight">{tab.label}</span>
            </button>
          );
        })}
      </div>
